...
function main() {
    return new Promise(async (resolve, reject) => {
        ...
        for (let item of module.exports.industries) {
            let check = await conn(tables.industries).where('token', item.token).first();
            ...
            if (!check) {
                ...
                await conn(tables.industries).where('id', check.id).update(item);
            }
        }
        ...
        for (let category of module.exports.roles) {
            for (let item of category.positions) {
                let check = await conn(tables.roles).where('token', item.token).first();
                ...
                if (!check) {
                    ...
                    await conn(tables.roles).where('id', check.id).update(item);
                }
            }
        }
